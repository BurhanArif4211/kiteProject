...
from .scorealgo_new import scorify
import random
import base64
import datetime
from firebase_admin import auth
from ErrorCodes import STATUS_CODES
from .services.firebase import upload_image,getPublicUrl,validateLogin, fireauth, storage, store


#(TODO) => { in profile creation time we need another element geoloc : [int,int] ~ [longitude,latitude] }
# Junk JS code here 
...
#  Okay bro we will add it later  

# ✔️ * DONE * (TODO) => { in profile creation time we need to create  /<publicProfileID>/pp.jpg ! /<userId>/pp.jpg  pls also update ours profiles } 

#(TODO) => { followers and likers must be submitted as likes:[[displayname,userId]] so we don't need to fetch db for each user's name } 
#(TODO) => { Commit and tell me about }
...
def index(request):
    if "user_data" in request.session:
        ...
        profile_data = store.collection('users1').where(
        ...
        profile_data_of_currently_logged_in_user = store.collection(
...
def createProfile(request):
    if request.method == 'POST' and 'user_data' in request.session:
        ...
        try:
            ...
            messages.success(
                request, ("Profile Created! Now Lets Get to know about each other."))
            return redirect("/kite")
        ...
        store.collection('posts1').document(postId).set(
            {
                ...
                'likes': [],
                # add comments later by using update method in firebase //It should be better to use Array e.g [{name:aman, caption: caption, time: time},{name: so on}] or how'll you name elements of dict for each comments?
                'comments': [],
                # TODO IMPORTANT: add a date fetching system from server since users can expliot this!!!
...
def loadUserPosts(request):
    # For security so the user can't load his posts separately!
    if 'user_data' in request.session and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        encoded_user_data = request.session['user_data']
        decoded_user = base64.b64decode(encoded_user_data).decode()
        try:
            claims = auth.verify_id_token(decoded_user)
        ...
        return TemplateResponse(request, 'kite/posts-grid.html', {'posts': posts_info, 'default': {
            'pic': profile_data_of_currently_logged_in_user.get('pp_url'), 'name': auth.get_user(claims['user_id']).display_name
        }, })
...
def like(request, url):
    if 'user_data' in request.session:
        ...
        post_query = store.collection('posts1').where(
            'post_id', '==', url).get()
...
def followUserByPublicId(request, publicProfileId):
    if 'user_data' in request.session:
        ...
        print(
            f"the follow function was called by {claims['name']} for {publicProfileId}")

        ours_profile_data = store.collection('users1').where(
            'user_id', '==', claims['user_id']).get()
        ourPublicProfileId = ours_profile_data[0].to_dict().get(
            'publicProfileId')
        ...
        if publicProfileId == ourPublicProfileId:
            ...
            user_query = store.collection('users1').where(
                'publicProfileId', '==', ourPublicProfileId).stream()
            for doc in user_query:
                store.collection('users1').document(doc.id).update(
                    {'following': list(ourCurrentFollowing)})

            # Update 'followers' for the other user
            user_query = store.collection('users1').where(
                'publicProfileId', '==', publicProfileId).stream()
            for doc in user_query:
                user_data = doc.to_dict()
                ...
                if ourPublicProfileId in their_current_followers:
                    ...
                    store.collection('users1').document(doc.id).update(
                        {'followers': list(their_current_followers)})
...
# Be confident About your self. One day you are gonna be proud of your self!
def kitePGHelper(req, publicProfileId, claims):
    user_profile_data = store.collection('users1').where(
    ...
    if len(user_profile_data) > 0:
        ...
        user_data = auth.get_user(
            user_profile_data[0].to_dict().get('user_id'))
        print(user_data)
        user_info = {
        ...
        # render(req, "authentication/profile-form.html")  # I dont understand why you rendered this form here.
        return redirect('/kite')
